...
export class DashboardComponent implements OnInit {
  ...
  // View the details of the issue by navigating to the issue details page
  viewDetails(id: number): void {
    console.log(`Viewing details for issue with ID: ${id}`);
    this.router.navigate([`/issue-details/${id}`]);  // Assuming you have a route like this set up
  }
  ...
  deleteIssue(id: number): void {
    ...
    this.issueService.deleteIssue(id).subscribe(() => {
      // Reload the issues list after deletion
      this.loadIssues();
    });
  }
}
